...
const CreateCampaign: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<CreateCampaignForm> = async (values) => {
    ...
    const imageCid = await web3StorageClient.put(values.image);
    const bannerCid = await web3StorageClient.put(values.banner);
    const registeredImageCid = await web3StorageClient.put(values.registeredImage);
    const unfinishedImageCid = await web3StorageClient.put(values.unfinishedImage);

    const imageURL = createFilePath(imageCid, values.image[0].name);
    const bannerURL = createFilePath(bannerCid, values.banner[0].name);
    const registeredImageURL = createFilePath(registeredImageCid, values.registeredImage[0].name);
    const unfinishedImageURL = createFilePath(unfinishedImageCid, values.unfinishedImage[0].name);
