...
#[ink::contract]
pub mod delegator {
    use ink::{env::CallFlags, storage::Mapping};
    ...
    mod e2e_tests {
        ...
        #[ink_e2e::test]
        async fn e2e_counter_mutated<Client: E2EBackend>(mut client: Client) -> E2EResult<()> {
            // given
            let origin = client
        ...
        #[ink_e2e::test]
        async fn e2e_mapping_mutated<Client: E2EBackend>(mut client: Client) -> E2EResult<()> {
            let origin = client
                .create_and_fund_account(&ink_e2e::alice(), 10_000_000_000_000)
                .await;
